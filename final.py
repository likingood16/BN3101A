...
PPG_PEAK_PROM       = 0.28
PPG_Z_THR           = 5        # CHANGED: Was 4.2 (even less sensitive to outliers)
PPG_DEV_FRAC        = 1      # CHANGED: Was 0.80 (even less sensitive to fractional deviation)
PPG_REQUIRE_BOTH    = True     # amp AND slope anomalies required
PPG_MIN_BEAT_AMP    = 0.35
PPG_RMSE_ENABLE     = True
PPG_RMSE_THR        = 1      # CHANGED: Was 0.65 (less sensitive to individual beat shape)
PPG_WORST_PCT       = 0.25
...
PPG_REGULAR_RMSE_P80      = 0.80    # CHANGED: Was 0.60 (This is the KEY change. It allows more "ugly" signals to be classed as regular)
PPG_MIN_FLAGGED_BEATS     = 6       # CHANGED: Was 5 (requires more bad beats to alert)
PPG_MIN_FLAGGED_FRAC      = 0.30    # CHANGED: Was 0.25 (requires higher % of bad beats to alert)
...
class DualSignalVisualizationGUI(QtWidgets.QMainWindow):
    ...
    def update_plots(self):
        ...
        # --- MODIFICATION: DETECTION LOGIC MOVED TO TOP ---
        # Get the data segment for detection
        samples_to_show = int(WINDOW_DURATION * SAMPLING_RATE)
        ppg_seg_detect = ppg[-samples_to_show:]
        eeg_seg_detect = eeg[-samples_to_show:]

        # Run detection regardless of view mode.
        # We still need 'ppg_spans' to draw the ellipses in waveform view
        # and 'eeg_spans' for the dynamic EEG alert.
        ppg_spans = self.detect_ppg_irregular_beats(ppg_seg_detect, SAMPLING_RATE)
        eeg_spans = self.detect_irregular_spans_eeg(eeg_seg_detect, SAMPLING_RATE)

        # --- THIS IS THE KEY CHANGE ---
        # Force the PPG alert to be permanently active as requested
        self.ppg_alert_active = True
        # Keep the EEG alert dynamic based on its detection
        self.eeg_alert_active = len(eeg_spans) > 0
        # --- END OF MODIFICATION ---

        if self.view_mode == 'waveform':
            # Note: ppg_seg/eeg_seg are the same as ppg_seg_detect/eeg_seg_detect
            ...
            # --- OLD DETECTION LOGIC WAS HERE (NOW REMOVED) ---
            
            # Helper function for drawing ellipses
            ...
            # Add ellipses based on ppg_spans (calculated at the top)
            ...
            # self.ppg_alert_active is now always True
            ...
            # This now uses the self.ppg_alert_active variable set at the top
